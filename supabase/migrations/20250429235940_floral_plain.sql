/*
  ...
  1. New Tables
    - `ai_conversation_logs` - Stores detailed information about AI interactions
      - Tracks prompts, responses, confidence scores, and user feedback
      - Includes performance metrics like tokens used and processing time
      - Links to conversations and AI agents

  2. Functions
    - `calculate_ai_confidence()` - Calculates average confidence for a conversation
    - `trigger_ai_handoff()` - Handles transferring conversations from AI to human agents
    - `analyze_ai_performance()` - Generates performance metrics for specific AI agents
    - `generate_ai_metrics_snapshot()` - Creates periodic snapshots of AI performance

  3. Security
    - RLS enabled with appropriate policies
    - Access restricted to users with metrics viewing permission or admin role
*/
