/*
  # Fix AI Agent Settings and Webhook Events Policies

  1. Changes
    - Drop all existing policies on ai_agent_settings and ai_webhook_events tables
    - Create new policies with timestamped names to avoid conflicts
    - Fix function definitions and improve safety
    - Ensure default settings for existing AI agents
...
DROP POLICY IF EXISTS "AI agent settings readable by authenticated" ON ai_agent_settings;
DROP POLICY IF EXISTS "AI agent settings manageable by admins" ON ai_agent_settings;
DROP POLICY IF EXISTS "AI agent settings readable by authenticated_20250502" ON ai_agent_settings;
DROP POLICY IF EXISTS "AI agent settings manageable by admins_20250502" ON ai_agent_settings;
...
DROP POLICY IF EXISTS "AI webhook events readable by authorized" ON ai_webhook_events;
DROP POLICY IF EXISTS "AI webhook events readable by authorized_20250502" ON ai_webhook_events;
