/*
  # AI Agent Settings and Webhook Handler

  1. New Tables
    - ai_agent_settings: Stores detailed configuration for AI agents
    - ai_webhook_events: Tracks AI-specific webhook events
  
  2. New Functions
    - get_ai_agent_settings: Retrieves settings for a specific AI agent
    - update_ai_agent_performance: Updates performance metrics for AI agents
    - trigger_ai_webhook: Sends webhook notifications for AI events
...
-- Create policies for ai_agent_settings only if they don't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'ai_agent_settings' 
    AND policyname = 'Authenticated users can read AI agent settings'
  ) THEN
    EXECUTE 'CREATE POLICY "Authenticated users can read AI agent settings" 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'ai_agent_settings' 
    AND policyname = 'Only admins and managers can manage AI agent settings'
  ) THEN
    EXECUTE 'CREATE POLICY "Only admins and managers can manage AI agent settings"
...
-- Create policies for ai_webhook_events only if they don't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'ai_webhook_events' 
    AND policyname = 'Authenticated users can read AI webhook events'
  ) THEN
    EXECUTE 'CREATE POLICY "Authenticated users can read AI webhook events"
...
-- Create function to get AI agent settings
...
-- Create function to trigger AI webhook with fixed loop variable syntax
...
RETURNS void AS $$
DECLARE
  webhook_record RECORD; -- Use a RECORD type for the loop variable
BEGIN
  ...
  LOOP
    ...
    VALUES (
      webhook_record.id, -- Access the id field of the record
      event_type,
      jsonb_build_object(
